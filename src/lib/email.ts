...
/**
 * Single From identity for all Kliv∞ mail. Override with EMAIL_FROM
 * (e.g. "Kliv∞ <noreply@kliv∞.in>"). The domain part must be verified
 * in Resend or every send fails with a 403.
 */
const FROM = process.env.EMAIL_FROM || 'Kliv∞ <noreply@kliv∞.in>'

/** Where login / app links point — the actual app, always the production origin. */
const APP_URL = 'https://app.kliv∞.in'
...
async function dispatch(opts: { to: string; subject: string; html: string; text: string }): Promise<void> {
  ...
  if (error) {
    // e.g. "The kliv∞.in domain is not verified" (403), invalid recipient
  ...
  <div style="margin:0;padding:24px 12px;background:${COLOR.canvas};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid ${COLOR.border};border-radius:18px;overflow:hidden;box-shadow:0 10px 40px -8px rgba(28,25,23,0.12);">
        ...
        <tr><td style="padding:20px 28px;border-bottom:1px solid ${COLOR.border};">
          <table role="presentation" cellpadding="0" cellspacing="0"><tr>
            <td style="vertical-align:middle;padding-right:10px;">
              <img src="${APP_URL}/logo.png" width="30" height="30" alt="Kliv∞" style="display:block;border-radius:7px;" />
            </td>
            <td style="vertical-align:middle;">
              <span style="font-size:20px;font-weight:800;letter-spacing:-0.5px;color:${COLOR.textDark};">Kliv∞</span>
            </td>
          </tr></table>
        </td></tr>
      </table>
      ...
      <div style="padding:20px 28px;border-top:1px solid ${COLOR.border};text-align:center;color:${COLOR.textMuted};font-size:12px;line-height:1.6;">
        <p style="margin:0;">© 2026 Kliv∞ — run your freelance business without the chaos.</p>
        ${waitlistFooter ? `<p style="margin:6px 0 0;">You're receiving this because you joined the Kliv∞ waitlist.</p>` : ''}
      </div>
    </div>
  </div>`
}
...
export async function sendWaitlistConfirmation(email: string, name?: string): Promise<void> {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;">Thanks for joining the Kliv∞ waitlist — we've saved your spot.</p>
    ...
    <p style="margin:0 0 10px;font-weight:700;color:${COLOR.textDark};">What you'll get with Kliv∞</p>
  ...
  const text = `You're on the Kliv∞ waitlist!

Hi ${name?.trim() || 'there'},

Thanks for joining the Kliv∞ waitlist — we've saved your spot. As an early member, the first 100 users get 1 month of the Pro plan free.

We'll review your request and email your login details as soon as you're approved (usually 24–48 hours).

What you'll get with Kliv∞:
...
© 2026 Kliv∞`
  ...
  await dispatch({
    to: email,
    subject: "You're on the Kliv∞ waitlist 🎉",
...
export async function sendWaitlistApproval(
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;">Great news — your Kliv∞ waitlist request has been approved and your account is ready.</p>
    ...
    <div style="text-align:center;margin:0 0 4px;">
      ${ctaButton(loginUrl, 'Log in to Kliv∞ →')}
    </div>
  `

  const text = `You're approved! Your Kliv∞ account is ready.
...
© 2026 Kliv∞`
  ...
  await dispatch({
    to: email,
    subject: "You're approved! Your Kliv∞ account is ready ✅",
    html: shell(body, { waitlistFooter: true }),
    text,
  })
}
...
/**
 * Notify an approved applicant who already had a Kliv∞ account — no
...
export async function sendWaitlistApprovalExisting(email: string, plan: string): Promise<void> {
  ...
  const body = `
    ...
    <p style="margin:0 0 24px;">
      Good news — your Kliv∞ account has been upgraded to the
  ...
  const text = `Your Kliv∞ plan has been upgraded.
...
© 2026 Kliv∞`
  ...
  await dispatch({
    to: email,
    subject: 'Your Kliv∞ plan has been upgraded ✅',
    html: shell(body),
    text,
  })
}
...
export async function sendTeamMemberCredentials({
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;">Hi ${safeName}, <strong>${safeOrg}</strong> has added you to their workspace on Kliv∞.</p>
  ...
  const text = `Welcome to ${orgName} on Kliv∞

Hi ${toName}, ${orgName} has added you to their workspace on Kliv∞.
...
© 2026 Kliv∞`
  ...
  await dispatch({
    to: toEmail,
    subject: `You've been added to ${orgName} on Kliv∞`,
    html: shell(body),
    text,
  })
}
